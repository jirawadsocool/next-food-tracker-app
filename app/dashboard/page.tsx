...
const DashboardPage: React.FC = () => {
  ...
  const handleEdit = (id: number) => {
    console.log(`Navigating to update food item with ID: ${id}`);
    window.location.href = `/updatefood/${id}`;
  };

  const handleLogout = () => {
    // Navigate to the login page using standard browser navigation
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <a href="/profile" className="bg-pink-500 text-white font-bold py-3 px-6 rounded-full shadow-lg transform transition-transform duration-300 hover:scale-105 hover:bg-pink-600 focus:outline-none focus:ring-4 focus:ring-pink-500 focus:ring-opacity-50">
              Edit Profile
            </a>
            <button
              onClick={handleLogout} // Attach the logout handler
              className="bg-purple-500 text-white font-bold py-3 px-6 rounded-full shadow-lg transform transition-transform duration-300 hover:scale-105 hover:bg-purple-600 focus:outline-none focus:ring-4 focus:ring-purple-500 focus:ring-opacity-50">
              Logout
            </button>
          </div>
        </div>
